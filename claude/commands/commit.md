---
allowed-tools: Bash(git add:*), Bash(git status:*), Bash(git commit:*), Bash(git diff:*), Bash(git log:*)
description: 日本語で詳細なコミットメッセージを生成してコミットする
---

変更内容を分析し、適切な粒度でコミットを分割して自動コミットする。

**すべての出力は日本語で記述すること。**

## コンテキスト収集

以下のコマンドを実行して現在の状態を把握する：

- `git status` で変更状態を確認
- `git diff HEAD` でステージング済み・未ステージングの差分を取得
- `git branch --show-current` で現在のブランチを確認
- `git log --oneline -10` で直近のコミット履歴を確認

## 手順

1. 差分全体を分析し、論理的にまとまりのある単位（機能追加、バグ修正、リファクタリングなど）にグループ分けする
   - 1つのコミットには1つの目的・意図のみを含める
   - 関連しない変更は別コミットに分ける

2. グループごとに以下を繰り返す：

   a. `git add -p` を使って、該当グループに属する変更のみをステージングする
      - `-p`（パッチモード）で hunk 単位で選択的にステージングする
      - 新規ファイルは `git add <file>` でステージングする
      - `.env`、認証情報、シークレットを含むファイルはステージングしない

   b. コミットメッセージを生成する
      - Conventional Commits形式を使用（feat:, fix:, docs:, refactor:, test:, chore:, perf:, ci:）
      - 1行目: タイプとタイトル（日本語、簡潔に）
      - 2行目: 空行
      - 3行目以降: 変更内容の詳細（日本語、箇条書き）
      - 最後に「なぜこの変更が必要か」の理由を記述する

      例：
      ```
      feat: ユーザープロフィール編集機能を追加

      - プロフィール編集フォームコンポーネントを実装
      - バリデーション（メール形式、名前の文字数制限）を追加
      - 編集成功時のトースト通知を実装

      理由: ユーザーが自身のプロフィール情報を更新できるようにするため
      ```

   c. 確認を求めず、生成したメッセージで `git commit` を自動実行する

3. すべてのグループをコミットし終えたら、コミット一覧を表示する

## 制約事項

- Co-Authored-By フッターを追加しない
- コミットメッセージは日本語で記述する（タイプ接頭辞は英語）
- `--no-verify` フラグを使用しない
- pre-commit hookが失敗した場合は、問題を修正してから新しいコミットを作成する
